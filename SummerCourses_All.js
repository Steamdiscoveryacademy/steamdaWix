...
// console.log("isBrad: " + isBrad);//maybe later...
const isPreview = wixWindow.viewMode === "Preview" ? true : false;
// console.log("isPreview: " + isPreview);//maybe later...
...
$w.onReady(function () {
    ...
    if (Number($w('#gradeleveldropdown').value) + 0 === -7) {
        // console.warn("ONREADY: " + Number(Number($w('#gradeleveldropdown').value) + 0 ) + " => gradeleveldropdown_change()" );//maybe later...
        gradeleveldropdown_change();
    }
    const query = wixLocation.query;
    // if (query) {
    //     // console.log("GET: " + JSON.stringify(query));//maybe later...
    // }else{
    //     // console.log("GET: " + 'No URL Parameters');
    // }
    // console.log("Base URL: " + wixLocation.baseUrl);//maybe later...
    // console.warn("COMPLETE: " + "$w.onReady(function ()" );//maybe later...
});
...
export function gradeleveldropdown_changeFull(event) {
    ...
    // console.log("gradelevelValue: " + gradelevelValue);//maybe later...
    if (gradelevelValue <= 0) {
        // console.log("All Grade Levels[" + gradelevelValue + "]: RESET FILTERS" );//maybe later...
        //<WHERE>
        ...
        if (gradelevelValue === -7) {
            // console.log("[" + gradelevelValue + "] timeout start: " + msSpan)//maybe later...
        ...
        // console.log("Specific Grade Level[" + gradelevelValue + "]: FILTER BY GRADE LEVEL" );//maybe later...
        //<WHERE>
        $w("#week1Courses").setFilter(wixData.filter()
        ...
        setTimeout(refreshRepeaterAllWeeks, msSpan);
        // setTimeout(console.log,msSpan, "Faux TimeOut");
        // console.warn("After grade filter, then 10 seconds, COMPLETE: " + "gradeleveldropdown_change(event)" );//maybe later...
    }
...
export function courseNameWk3_clickX(event, $w) {
    ...
    // console.warn("url: " + url);//maybe later...
    wixLocation.to(url);
}
...
export function week4courses_ready() {
    // This function was added from the Properties & Events panel. To learn more, visit http://wix.to/UcBnC-4
    // Add your code for this event here:
    // console.log("[week4courses_ready()] about to run refreshRepeaterAllWeeks ")//maybe later...
    // refreshRepeaterAllWeeks();
    // console.warn("COMPLETE: " + "week4courses_ready()" );
    // console.warn("SKIPPED: " + "week4courses_ready() will add to normal onReady()" );//maybe later...
}
...
export function refreshRepeaterAllWeeks() {
    ...
    // console.warn("COMPLETE: " + "refreshRepeaterAllWeeks()" );//maybe later...
}
